...
export default function ConfirmPage() {
  ...
  async function confirm() {
    if (!selected) return;
    setBusy(true);
    notify("pending", "Confirm completion: waiting…");
    try {
      const addr = await detectWallet();
      if (addr) setWalletAddr(addr);
      const result = await writeContract("confirm_completion", [selected.id]);
  ...
  const canConfirm = selected
    ...
    && selected.delivery_deadline > nowSec
    && selected.courier_delivery_checkpoint > 0;
  ...
  return (
    <div>
      ...
      <div className="form-card">
        <label>
          Select your delivery (IN_TRANSIT or DELIVERED)
        ...
        <button className="green-btn full" disabled={!canConfirm || !!busy} onClick={confirm}>
          {busy ? "Processing…" : !walletAddr ? "Wallet not connected" : deliveries.length === 0 ? "No eligible deliveries" : !selected ? "Select a delivery" : selected.status !== "IN_TRANSIT" && selected.status !== "DELIVERED" ? `Status: ${selected.status}` : selected.courier_delivery_checkpoint <= 0 ? "No courier delivery checkpoint" : selected.delivery_deadline <= nowSec ? "Deadline passed" : "Confirm completion"}
        </button>
      </div>
      ...
      {selected && (
        <div className="state-card" style={{ marginTop: 24 }}>
          ...
          <dl>
            ...
            <dt>Courier delivery checkpoint</dt><dd>{selected.courier_delivery_checkpoint > 0 ? `#${selected.courier_delivery_checkpoint}` : "None"}</dd>
      ...
      {!selected && !loadingList && (
        <div className="empty-state" style={{ marginTop: 24 }}>
          <ShieldCheck />
          <p>{!walletAddr ? "Connect your wallet to see your deliveries." : "No IN_TRANSIT or DELIVERED deliveries found for your wallet."}</p>
        </div>
      )}
